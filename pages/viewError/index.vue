      ...
      <div class="mb-5">
        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3">
              <nuxt-link to="/viewError" class="btn btn-info">處理中</nuxt-link>
            </li>
            <li class="mr-auto">
              <nuxt-link to="/viewError/done" class="btn btn-outline-info"
                >已完成</nuxt-link
              >
            </li>
          </ul>
  ...
  methods: {
    ...
    async getAllTickets(page = 1) {
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/block-camera?status=0&relation=received&page=${page}`
      });
      if (data.success) {
