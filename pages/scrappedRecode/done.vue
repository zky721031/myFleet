      ...
      <div class="mb-5">
        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3">
              <nuxt-link to="/scrappedRecode" class="btn btn-outline-info"
                >處理中</nuxt-link
              >
            </li>
            <li class="mr-auto" @click="getAllDoneTickets()">
              <nuxt-link to="/scrappedRecode/done" class="btn btn-info"
                >已完成</nuxt-link
              >
            </li>
            <li class="mr-2">
  ...
  methods: {
    ...
    async getSearchData(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/scrap?status=1&relation=send&page=${page}&ticket_id=${this.searchTicketId}&vehicle=${this.searchVehicle}&station=${this.searchStation}&start_dt=${this.searchStartTime}&end_dt=${this.searchEndTime}`
      });
      if (data.success) {
    ...
    async getAllDoneTickets(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/scrap?status=1&relation=send&page=${page}`
      });
      if (data.success) {
        this.allDoneTickets = data.results.tickets;
        this.allDoneTicketsPagination = data.results.pagination;
