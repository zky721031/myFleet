        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3" @click="getAllTickets()">
              <nuxt-link to="/admin/msgInstall" class="btn btn-info"
                >處理中</nuxt-link
              >
            </li>
            <li class="mr-auto">
              <nuxt-link
                to="/admin/msgInstall/done"
  ...
  methods: {
    ...
    async getSearchData(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/new-vehicle?status=0&relation=received&page=${page}&ticket_id=${this.searchTicketId}&vehicle=${this.searchVehicle}&station=${this.searchStation}&start_dt=${this.searchStartTime}&end_dt=${this.searchEndTime}`
      });
      if (data.success) {
    ...
    async getAllTickets(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/new-vehicle?status=0&relation=received&page=${page}`
      });
      if (data.success) {
        this.allTickets = data.results.tickets;
        this.allTicketsPagination = data.results.pagination;
