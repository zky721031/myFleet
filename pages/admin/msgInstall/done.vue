      ...
      <div class="mb-5">
        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3">
              <nuxt-link to="/admin/msgInstall" class="btn btn-outline-info"
                >處理中</nuxt-link
              >
            </li>
            <li class="mr-auto" @click="getAllDoneTickets()">
              <nuxt-link to="/admin/msgInstall/done" class="btn btn-info"
                >已完成</nuxt-link
              >
            </li>
            <li class="mr-2">
            ...
            >
              <li class="p-3 mb-3 rounded bg_gray li_span list_effect">
                ...
                <span class="four"
                  ...
                  >{{ ticket.station }}</span
                >
                <span class="five">{{ ticket.vehicle_type }}</span>
                <span
                  ><i class="fas fa-comment-dots mr-1 icons_gray"></i
                  >{{ `${ticket.content.slice(0, 20)}...` }}</span
                >
...
export default {
  ...
  methods: {
    // url: `/tickets/new-vehicle?status=1&relation=received&page=${page}`
    ...
    async getSearchData(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/new-vehicle?status=1&relation=received&page=${page}&ticket_id=${this.searchTicketId}&vehicle=${this.searchVehicle}&station=${this.searchStation}&start_dt=${this.searchStartTime}&end_dt=${this.searchEndTime}`
      });
      if (data.success) {
