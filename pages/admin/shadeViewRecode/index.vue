      ...
      <div class="mb-5">
        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3">
              <nuxt-link to="/admin/shadeViewRecode" class="btn btn-info"
                >處理中</nuxt-link
              >
            </li>
            <li class="mr-auto">
              <nuxt-link
                to="/admin/shadeViewRecode/done"
        ...
        <div>
          <ul class="ul_style_none mb-5 pl-0">
            <nuxt-link
              ...
              :to="{
                name: 'admin-shadeViewRecode-id___zh',
                params: {
                  id: ticket.ticket_id
                },
                query: {
  ...
  methods: {
    ...
    async getAllTickets(page = 1) {
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/block-camera?status=0&relation=send&page=${page}`
      });
      if (data.success) {
