      ...
      <div class="mb-5">
        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3">
              <nuxt-link to="/admin/msgVideoRead" class="btn btn-outline-info"
                >處理中</nuxt-link
              >
            </li>
            <li class="mr-auto" @click="getAllDoneTickets()">
              <nuxt-link to="/admin/msgVideoRead/done" class="btn btn-info"
                >已完成</nuxt-link
              >
            </li>
            <li class="mr-2">
        ...
        <div>
          <ul class="ul_style_none mb-5 pl-0">
            <nuxt-link
              ...
              :to="{
                name: 'admin-msgVideoRead-id___zh',
                params: {
                  id: ticket.ticket_id
                },
                query: {
                  ...
                  start_dt: ticket.start_dt,
                  end_dt: ticket.end_dt,
            ...
            >
              <li class="p-3 mb-3 rounded bg_gray li_span list_effect">
                ...
                <span class="mr-2"
                  ...
                  ><small
                    >{{ ticket.start_dt }}~{{ ticket.end_dt }}</small
                  ></span
                >
                <span
                  ><i class="fas fa-comment-dots mr-1 icons_gray"></i
                  >{{ `${ticket.content.slice(0, 8)}...` }}</span
                >
  ...
  methods: {
    ...
    async getSearchData(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.api,
        url: `/tickets/raw-video?status=1&relation=received&page=${page}&ticket_id=${this.searchTicketId}&vehicle=${this.searchVehicle}&station=${this.searchStation}&start_dt=${this.searchStartTime}&end_dt=${this.searchEndTime}`
      });
      if (data.success) {
    ...
    async getAllDoneTickets(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.api,
        url: `/tickets/raw-video?status=1&relation=received&page=${page}`
      });
      if (data.success) {
        this.allDoneTickets = data.results.tickets;
        this.allDoneTicketsPagination = data.results.pagination;
