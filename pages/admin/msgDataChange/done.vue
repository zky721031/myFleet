      ...
      <div class="mb-5">
        ...
        <div class="mb-4">
          <ul class="d-flex align-items-center pl-0 mb-0 ul_style_none">
            <li class="mr-3">
              <nuxt-link to="/admin/msgDataChange" class="btn btn-outline-info"
                >處理中</nuxt-link
              >
            </li>
            <li class="mr-auto" @click="getAllDoneTickets()">
              <nuxt-link to="/admin/msgDataChange/done" class="btn btn-info"
                >已完成</nuxt-link
              >
            </li>
            <li class="mr-2">
        ...
        <div>
          <ul class="ul_style_none mb-5 pl-0">
            <nuxt-link
              ...
              :to="{
                name: 'admin-msgDataChange-id___zh',
                params: {
                  id: ticket.ticket_id
                },
                query: {
  ...
  methods: {
    ...
    async getSearchData(page = 1) {
      ...
      const { data } = await this.$axios({
        method: "GET",
        baseURL: this.Api,
        url: `/tickets/vehicle-change?status=1&relation=received&page=${page}&ticket_id=${this.searchTicketId}&vehicle=${this.searchVehicle}&station=${this.searchStation}&start_dt=${this.searchStartTime}&end_dt=${this.searchEndTime}`
      });
      if (data.success) {
